...
function CreateDataDir {
  ...
  echo "rpcwait=1" >> $CONF
  echo "walletnotify=${LYRAANDWAIT} -STOP" >> $CONF
  shift
  while (( "$#" )); do
      echo $1 >> $CONF
      shift
...
function Lyra {
  ...
  amount=$3
  address=$(Address $to)
  txid=$( ${LYRAANDWAIT} $CLI $from lyratoaddress $address $amount )
}
...
# Use: LyraRawTxn <datadir> <hex_txn_data>
function LyraRawTxn {
  ${LYRAANDWAIT} $CLI $1 lyrarawtransaction $2
}
