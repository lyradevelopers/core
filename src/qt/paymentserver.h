...
// but the main window (and the Lyra Coins tab) is not.
//
// So, the strategy is:
//
...
public:
    ...
    static bool ipcLyraCommandLine();
...
signals:
    ...
    void receivedPaymentRequest(LyraCoinsRecipient);
...
public slots:
    ...
    void fetchPaymentACK(CWallet* wallet, LyraCoinsRecipient recipient, QByteArray transaction);
...
private:
    bool processPaymentRequest(PaymentRequestPlus& request, LyraCoinsRecipient& recipient);
