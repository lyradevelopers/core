                                          ...
                                          walletModel(0)
{
    ...
    lyraCoinsPage = new LyraCoinsDialog();

    addWidget(overviewPage);
    addWidget(transactionsPage);
    addWidget(receiveCoinsPage);
    addWidget(lyraCoinsPage);
    ...
    // Pass through messages from lyraCoinsPage
    connect(lyraCoinsPage, SIGNAL(message(QString, QString, unsigned int)), this, SIGNAL(message(QString, QString, unsigned int)));
...
{
    if (gui) {
        // Clicking on a transaction on the overview page simply lyras you to transaction history page
...
void WalletView::setClientModel(ClientModel* clientModel)
{
    ...
    lyraCoinsPage->setClientModel(clientModel);
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void WalletView::setWalletModel(WalletModel* walletModel)
{
    ...
    lyraCoinsPage->setModel(walletModel);
...
void WalletView::gotoLyraCoinsPage(QString addr)
{
    setCurrentWidget(lyraCoinsPage);

    if (!addr.isEmpty())
        lyraCoinsPage->setAddress(addr);
}
...
void WalletView::gotoMultiLyraDialog()
{
    MultiLyraDialog* multiLyraDialog = new MultiLyraDialog(this);
    multiLyraDialog->setModel(walletModel);
    multiLyraDialog->show();
}

bool WalletView::handlePaymentRequest(const LyraCoinsRecipient& recipient)
{
    return lyraCoinsPage->handlePaymentRequest(recipient);
}
...
void WalletView::usedLyraingAddresses()
{
    if (!walletModel)
        return;
    AddressBookPage* dlg = new AddressBookPage(AddressBookPage::ForEditing, AddressBookPage::LyraingTab, this);
