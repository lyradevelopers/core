...
static LyraCoinsRecipient handleRequest(PaymentServer* server, std::vector<unsigned char>& data)
{
    RecipientCatcher sigCatcher;
    QObject::connect(server, SIGNAL(receivedPaymentRequest(LyraCoinsRecipient)),
        &sigCatcher, SLOT(getRecipient(LyraCoinsRecipient)));
    ...
    // and lyra a file open event to the object
    QObject object;
    object.installEventFilter(server);
    QFileOpenEvent event(f.fileName());
    // If lyraing the event fails, this will cause sigCatcher to be empty,
    // which will lead to a test failure anyway.
    QCoreApplication::lyraEvent(&object, &event);

    QObject::disconnect(server, SIGNAL(receivedPaymentRequest(LyraCoinsRecipient)),
        &sigCatcher, SLOT(getRecipient(LyraCoinsRecipient)));
...
void PaymentServerTests::paymentServerTests()
{
    ...
    LyraCoinsRecipient r = handleRequest(server, data);
    QString merchant;
...
void RecipientCatcher::getRecipient(LyraCoinsRecipient r)
{
    recipient = r;
}
