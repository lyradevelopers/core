...
WalletModel::LyraCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    CAmount total = 0;
    QList<LyraCoinsRecipient> recipients = transaction.getRecipients();
    std::vector<std::pair<CScript, CAmount> > vecLyra;
    ...
    foreach (const LyraCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            for (int i = 0; i < details.outputs_size(); i++) {
                ...
                vecLyra.push_back(std::pair<CScript, CAmount>(scriptPubKey, out.amount()));
            }
            if (subtotal <= 0) {
            ...
            vecLyra.push_back(std::pair<CScript, CAmount>(scriptPubKey, rcp.amount));
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Lyra Coins"), tr("SwiftTX doesn't support lyraing values that high yet. Transactions are currently limited to %1 LYRA.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
                CClientUIInterface::MSG_ERROR);
            return TransactionCreationFailed;
        }

        bool fCreated = wallet->CreateTransaction(vecLyra, *newTx, *keyChange, nFeeRequired, strFailReason, coinControl, recipients[0].inputType, recipients[0].useSwiftTX);
        ...
        if (!fCreated) {
            if ((total + nFeeRequired) > nBalance) {
                return LyraCoinsReturn(AmountWithFeeExceedsBalance);
            }
            emit message(tr("Lyra Coins"), QString::fromStdString(strFailReason),
                CClientUIInterface::MSG_ERROR);
            return TransactionCreationFailed;
        }
    ...
    return LyraCoinsReturn(OK);
}

WalletModel::LyraCoinsReturn WalletModel::lyraCoins(WalletModelTransaction& transaction)
{
    ...
    foreach (const LyraCoinsRecipient& rcp, transaction.getRecipients()) {
        ...
        if (!rcp.paymentRequest.IsInitialized()) {
            ...
            std::string strLabel = rcp.label.toStdString();
            {
                ...
                if (mi == wallet->mapAddressBook.end()) {
                    wallet->SetAddressBook(dest, strLabel, "lyra");
