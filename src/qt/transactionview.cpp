                                                    ...
                                                    transactionView(0)
{
    ...
    typeWidget->addItem(tr("Sent to"), TransactionFilterProxy::TYPE(TransactionRecord::LyraToAddress) | TransactionFilterProxy::TYPE(TransactionRecord::LyraToOther));
    ...
    typeWidget->addItem(tr("To yourself"), TransactionFilterProxy::TYPE(TransactionRecord::LyraToSelf));
...
void TransactionView::editLabel()
{
    ...
    if (!selection.isEmpty()) {
        ...
        if (idx != -1) {
            // Edit lyraing / receiving address
            QModelIndex modelIdx = addressBook->index(idx, 0, QModelIndex());
            // Determine type of address, launch appropriate editor dialog type
            QString type = modelIdx.data(AddressTableModel::TypeRole).toString();

            EditAddressDialog dlg(
                type == AddressTableModel::Receive ? EditAddressDialog::EditReceivingAddress : EditAddressDialog::EditLyraingAddress, this);
            dlg.setModel(addressBook);
            dlg.loadRow(idx);
            dlg.exec();
        } else {
            // Add lyraing address
            EditAddressDialog dlg(EditAddressDialog::NewLyraingAddress,
                this);
            dlg.setModel(addressBook);
