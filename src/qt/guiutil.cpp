...
bool parseBitcoinURI(const QUrl& uri, LyraCoinsRecipient* out)
{
    // return if URI is not valid or is no LYRA: URI
    if (!uri.isValid() || uri.scheme() != QString(URI_SCHEME))
        return false;

    LyraCoinsRecipient rv;
    rv.address = uri.path();
...
bool parseBitcoinURI(QString uri, LyraCoinsRecipient* out)
{
...
QString formatBitcoinURI(const LyraCoinsRecipient& info)
{
...
void showLyraFolder(){
    boost::filesystem::path pathLyra = GetDataDir();

    /* Open folder with default browser */
    if (boost::filesystem::exists(pathLyra))
        QDesktopServices::openUrl(QUrl::fromLocalFile(boostPathToQString(pathLyra)));
}
void showBackups()
{
