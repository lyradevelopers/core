...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            connect(walletModel, SIGNAL(coinsSent(CWallet*, LyraCoinsRecipient, QByteArray)),
                paymentServer, SLOT(fetchPaymentACK(CWallet*, const LyraCoinsRecipient&, QByteArray)));
        }
#endif
        ...
        connect(paymentServer, SIGNAL(receivedPaymentRequest(LyraCoinsRecipient)),
            window, SLOT(handlePaymentRequest(LyraCoinsRecipient)));
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    // translated properly.
    if (PaymentServer::ipcLyraCommandLine())
        exit(0);
