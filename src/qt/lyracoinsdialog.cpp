...
#include "lyracoinsdialog.h"
#include "ui_lyracoinsdialog.h"
...
LyraCoinsDialog::LyraCoinsDialog(QWidget* parent) : QDialog(parent),
                                                    ui(new Ui::LyraCoinsDialog),
                                                    clientModel(0),
                                                    model(0),
                                                    ...
                                                    fFeeMinimized(true)
{
    ...
    if (!settings.contains("fLyraFreeTransactions"))
        settings.setValue("fLyraFreeTransactions", false);
    ...
    ui->checkBoxFreeTx->setChecked(settings.value("fLyraFreeTransactions").toBool());
    minimizeFeeSection(settings.value("fFeeSectionMinimized").toBool());
}

void LyraCoinsDialog::setClientModel(ClientModel* clientModel)
{
...
void LyraCoinsDialog::setModel(WalletModel* model)
{
    this->model = model;

    if (model && model->getOptionsModel()) {
        for (int i = 0; i < ui->entries->count(); ++i) {
            LyraCoinsEntry* entry = qobject_cast<LyraCoinsEntry*>(ui->entries->itemAt(i)->widget());
            if (entry) {
...
LyraCoinsDialog::~LyraCoinsDialog()
{
    QSettings settings;
    ...
    settings.setValue("fLyraFreeTransactions", ui->checkBoxFreeTx->isChecked());

    delete ui;
}

void LyraCoinsDialog::on_lyraButton_clicked()
{
    if (!model || !model->getOptionsModel())
        return;

    QList<LyraCoinsRecipient> recipients;
    bool valid = true;
    ...
    for (int i = 0; i < ui->entries->count(); ++i) {
        ...
        if (!model->isMine(address) && ui->splitBlockCheckBox->checkState() == Qt::Checked) {
            ...
            QMessageBox::warning(this, tr("Lyra Coins"),
                tr("The split block tool does not work when lyraing to outside addresses. Try again."),
    ...
    foreach (const LyraCoinsRecipient& rcp, recipients) {
    ...
    if (encStatus == model->Locked || encStatus == model->UnlockedForAnonymizationOnly) {
        ...
        lyra(recipients, strFee, formatted);
        return;
    }
...
void LyraCoinsDialog::lyra(QList<LyraCoinsRecipient> recipients, QString strFee, QStringList formatted)
{
    // prepare transaction for getting txFee earlier
    WalletModelTransaction currentTransaction(recipients);
    WalletModel::LyraCoinsReturn prepareStatus;
    ...
    processLyraCoinsReturn(prepareStatus,
    ...
    QString questionString = tr("Are you sure you want to lyra?");
    ...
    // Display message box
    QMessageBox::StandardButton retval = QMessageBox::question(this, tr("Confirm lyra coins"),
    ...
    // now lyra the prepared transaction
    WalletModel::LyraCoinsReturn lyraStatus = model->lyraCoins(currentTransaction);
    // process lyraStatus and on error generate message shown to user
    processLyraCoinsReturn(lyraStatus);

    if (lyraStatus.status == WalletModel::OK) {
        accept();
...
void LyraCoinsDialog::reject()
{
    clear();
}

void LyraCoinsDialog::accept()
{
    clear();
}

LyraCoinsEntry* LyraCoinsDialog::addEntry()
{
    LyraCoinsEntry* entry = new LyraCoinsEntry(this);
    entry->setModel(model);
    ui->entries->addWidget(entry);
    connect(entry, SIGNAL(removeEntry(LyraCoinsEntry*)), this, SLOT(removeEntry(LyraCoinsEntry*)));
...
void LyraCoinsDialog::updateTabsAndLabels()
{
    setupTabChain(0);
    coinControlUpdateLabels();
}

void LyraCoinsDialog::removeEntry(LyraCoinsEntry* entry)
{
    entry->hide();
...
QWidget* LyraCoinsDialog::setupTabChain(QWidget* prev)
{
    for (int i = 0; i < ui->entries->count(); ++i) {
        LyraCoinsEntry* entry = qobject_cast<LyraCoinsEntry*>(ui->entries->itemAt(i)->widget());
        if (entry) {
    ...
    QWidget::setTabOrder(prev, ui->lyraButton);
    QWidget::setTabOrder(ui->lyraButton, ui->clearButton);
    QWidget::setTabOrder(ui->clearButton, ui->addButton);
    return ui->addButton;
}

void LyraCoinsDialog::setAddress(const QString& address)
{
    LyraCoinsEntry* entry = 0;
    // Replace the first entry if it is still unused
    if (ui->entries->count() == 1) {
        LyraCoinsEntry* first = qobject_cast<LyraCoinsEntry*>(ui->entries->itemAt(0)->widget());
        if (first->isClear()) {
            entry = first;
        }
    }
...
void LyraCoinsDialog::pasteEntry(const LyraCoinsRecipient& rv)
{
...
bool LyraCoinsDialog::handlePaymentRequest(const LyraCoinsRecipient& rv)
{
...
void LyraCoinsDialog::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchBalance, const CAmount& watchUnconfirmedBalance, const CAmount& watchImmatureBalance)
{
...
void LyraCoinsDialog::updateDisplayUnit()
{
...
void LyraCoinsDialog::updateSwiftTX()
{
    QSettings settings;
...
void LyraCoinsDialog::processLyraCoinsReturn(const WalletModel::LyraCoinsReturn& lyraCoinsReturn, const QString& msgArg, bool fPrepare)
{
    ...
    // This comment is specific to LyraCoinsDialog usage of WalletModel::LyraCoinsReturn.
    // WalletModel::TransactionCommitFailed is used only in WalletModel::lyraCoins()
    // all others are used only in WalletModel::prepareTransaction()
    switch (lyraCoinsReturn.status) {
    ...
    case WalletModel::DuplicateAddress:
        msgParams.first = tr("Duplicate address found, can only lyra to each address once per lyra operation.");
        break;
    ...
    case WalletModel::AnonymizeOnlyUnlocked:
        // Unlock is only need when the coins are lyra
        if(!fPrepare)
            fAskForUnlock = true;
        else
    ...
    emit message(tr("Lyra Coins"), msgParams.first, msgParams.second);
}

void LyraCoinsDialog::minimizeFeeSection(bool fMinimize)
{
...
void LyraCoinsDialog::on_buttonChooseFee_clicked()
{
    minimizeFeeSection(false);
}

void LyraCoinsDialog::on_buttonMinimizeFee_clicked()
{
    updateFeeMinimizedLabel();
    minimizeFeeSection(true);
}

void LyraCoinsDialog::setMinimumFee()
{
    ui->radioCustomPerKilobyte->setChecked(true);
    ui->customFee->setValue(CWallet::minTxFee.GetFeePerK());
}

void LyraCoinsDialog::updateFeeSectionControls()
{
...
void LyraCoinsDialog::updateGlobalFeeVariables()
{
    ...
    fLyraFreeTransactions = ui->checkBoxFreeTx->isChecked();
}

void LyraCoinsDialog::updateFeeMinimizedLabel()
{
    if (!model || !model->getOptionsModel())
        return;
...
void LyraCoinsDialog::updateMinFeeLabel()
{
    if (model && model->getOptionsModel())
...
void LyraCoinsDialog::updateSmartFeeLabel()
{
    if (!model || !model->getOptionsModel())
        return;
...
// UTXO splitter
void LyraCoinsDialog::splitBlockChecked(int state)
{
    if (model) {
...
//UTXO splitter
void LyraCoinsDialog::splitBlockLineEditChanged(const QString& text)
{
...
void LyraCoinsDialog::coinControlButtonClicked()
{
...
void LyraCoinsDialog::coinControlChangeChecked(int state)
{
...
void LyraCoinsDialog::coinControlChangeEdited(const QString& text)
{
...
void LyraCoinsDialog::coinControlUpdateLabels()
{
...
void LyraCoinsDialog::on_pushButton_clicked()
{
    //grab the amount in Coin Control AFter Fee field
    QString qAfterFee = ui->labelCoinControlAfterFee->text().left(ui->labelCoinControlAfterFee->text().indexOf(" ")).replace("~", "").simplified().replace(" ", "");
    ...
    LyraCoinsEntry* entry = qobject_cast<LyraCoinsEntry*>(ui->entries->itemAt(0)->widget());
    if (entry) {
