...
{
public:
    explicit LyraCoinsRecipient() : amount(0), nVersion(LyraCoinsRecipient::CURRENT_VERSION) {}
    explicit LyraCoinsRecipient(const QString& addr, const QString& label, const CAmount& amount, const QString& message) : address(addr), label(label), amount(amount), message(message), nVersion(LyraCoinsRecipient::CURRENT_VERSION) {}
...
public:
    ...
    enum StatusCode // Returned by lyraCoins
    {
        OK,
        InvalidAmount,
        InvalidAddress,
    ...
    // Return status record for LyraCoins, contains error id + information
    struct LyraCoinsReturn {
        LyraCoinsReturn(StatusCode status = OK) : status(status) {}
        StatusCode status;
    };

    // prepare transaction for getting txfee before lyraing coins
    LyraCoinsReturn prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl = NULL);

    // Lyra coins to a list of recipients
    LyraCoinsReturn lyraCoins(WalletModelTransaction& transaction);
...
signals:
    ...
    void coinsSent(CWallet* wallet, LyraCoinsRecipient recipient, QByteArray transaction);
