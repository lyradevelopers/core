...
void CObfuscationPool::ProcessMessageObfuscation(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "dsa") { //Obfuscation Accept Into Pool
        ...
        if (dsq.ready) {
            ...
            if (pmn->nLastDsq != 0 &&
                pmn->nLastDsq + mnodeman.CountEnabled(ActiveProtocol()) / 5 > mnodeman.nDsqCount) {
                LogPrint("obfuscation", "dsq -- Masternode lyraing too many dsq messages. %s \n", pmn->addr.ToString());
                return;
            }
            mnodeman.nDsqCount++;
...
//
// Check the Obfuscation progress and lyra client updates if a Masternode
//
...
void CObfuscationPool::ChargeFees()
{
    ...
    if (state == POOL_STATUS_ACCEPTING_ENTRIES) {
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            // This queue entry didn't lyra us the promised transaction
            if (!found) {
                LogPrintf("CObfuscationPool::ChargeFees -- found uncooperative node (didn't lyra transaction). Found offence.\n");
                offences++;
            }
        }
    }
    ...
    if (state == POOL_STATUS_ACCEPTING_ENTRIES) {
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            // This queue entry didn't lyra us the promised transaction
            if (!found && r > target) {
                LogPrintf("CObfuscationPool::ChargeFees -- found uncooperative node (didn't lyra transaction). charging fees.\n");
...
//
void CObfuscationPool::LyraObfuscationDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, int64_t amount)
{
    if (fMasterNode) {
        LogPrintf("CObfuscationPool::LyraObfuscationDenominate() - Obfuscation from a Masternode is not supported currently.\n");
        return;
    }

    if (txCollateral == CMutableTransaction()) {
        LogPrintf("CObfuscationPool:LyraObfuscationDenominate() - Obfuscation collateral not set");
        return;
    }
    ...
    if (!sessionFoundMasternode) {
        LogPrintf("CObfuscationPool::LyraObfuscationDenominate() - No Masternode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return;
    }
    ...
    if (!CheckDiskSpace()) {
        ...
        LogPrintf("CObfuscationPool::LyraObfuscationDenominate() - Not enough disk space, disabling Obfuscation.\n");
        return;
    }

    UpdateState(POOL_STATUS_ACCEPTING_ENTRIES);

    LogPrintf("CObfuscationPool::LyraObfuscationDenominate() - Added transaction to pool.\n");
    ...
    if (!sessionFoundMasternode) {
        ...
        if (nUseQueue > 33) {
            ...
            BOOST_FOREACH (CObfuscationQueue& dsq, vecObfuscationQueue) {
                ...
                if (pnode != NULL) {
                    ...
                    LogPrintf("DoAutomaticDenominating --- connected (from queue), lyraing dsa for %d - %s\n", sessionDenom, pnode->addr.ToString());
                    strAutoDenomResult = _("Mixing in progress...");
        ...
        while (i < 10) {
            ...
            if (pnode != NULL) {
                ...
                pnode->PushMessage("dsa", sessionDenom, txCollateral);
                LogPrintf("DoAutomaticDenominating --- connected, lyraing dsa for %d\n", sessionDenom);
                strAutoDenomResult = _("Mixing in progress...");
                return true;
            } else {
...
bool CObfuscationPool::LyraRandomPaymentToSelf()
{
    ...
    // ****** Add fees ************ /
    vecLyra.push_back(make_pair(scriptChange, nPayment));

    CCoinControl* coinControl = NULL;
    bool success = pwalletMain->CreateTransaction(vecLyra, wtx, reservekey, nFeeRet, strFail, coinControl, ONLY_DENOMINATED);
    if (!success) {
        LogPrintf("LyraRandomPaymentToSelf: Error - %s\n", strFail);
        return false;
    }

    pwalletMain->CommitTransaction(wtx, reservekey);

    LogPrintf("LyraRandomPaymentToSelf Success: tx %s\n", wtx.GetHash().GetHex());
...
bool CObfuscationPool::MakeCollateralAmounts()
{
    ...
    vecLyra.push_back(make_pair(scriptCollateral, OBFUSCATION_COLLATERAL * 4));

    // try to use non-denominated and not mn-like funds
    bool success = pwalletMain->CreateTransaction(vecLyra, wtx, reservekeyChange,
    ...
    if (!success) {
        ...
        success = pwalletMain->CreateTransaction(vecLyra, wtx, reservekeyChange,
...
bool CObfuscationPool::CreateDenominated(int64_t nTotalValue)
{
    ...
    BOOST_REVERSE_FOREACH (int64_t v, obfuScationDenominations) {
        ...
        while (nValueLeft - v >= OBFUSCATION_COLLATERAL && nOutputs <= 10) {
            ...
            vecLyra.push_back(make_pair(scriptDenom, v));
    ...
    // if we have anything left over, it will be automatically lyra back as change - there is no need to lyra it manually
