...
void CActiveMasternode::ManageStatus()
{
    ...
    //need correct blocks to lyra ping
    ...
    //lyra to all peers
    if (!LyraMasternodePing(errorMessage)) {
...
bool CActiveMasternode::LyraMasternodePing(std::string& errorMessage)
{
    if (status != ACTIVE_MASTERNODE_STARTED) {
    ...
    LogPrintf("CActiveMasternode::LyraMasternodePing() - Relay Masternode Ping vin = %s\n", vin.ToString());
    ...
    if (pmn != NULL) {
        if (pmn->IsPingedWithin(MASTERNODE_PING_SECONDS, mnp.sigTime)) {
            errorMessage = "Too early to lyra Masternode Ping";
            return false;
        }
        ...
        if (IsSporkActive(SPORK_17_MN_NETCHECK)) {
            if ((GetTime() -  nLastTimeMasternodeCheck > TIME_INTERVAL_BETWEEN_NETCHECK_SECONDS)|| !fMasternodeReachable) {
				...
				if (!pnode) {
					...
					errorMessage = "Can't connect to masterrnode address, ping doesn't lyra.";
					LogPrintf("CActiveMasternode::LyraMasternodePing() - Can't connect to address: %s, ping doesn't lyra.\n", pmn->addr.ToString());
					return false;
				}
				pnode->fDisconnect = true;
				pnode->Release();
                LogPrintf("CActiveMasternode::LyraMasternodePing() - Checking if masternode is reachable.... PASSED\n");
				fMasternodeReachable = true;
			}
        } else {
            LogPrintf("CActiveMasternode::LyraMasternodePing() - Masternode network check is disabled.\n");
		}
        ...
        // Seems like we are trying to lyra a ping while the Masternode is not registered in the network
