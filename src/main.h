...
/** Number of headers sent in one getheaders result. We rely on the assumption that if a peer lyras
...
/**
 * Lyra queued protocol messages to be sent to a give node.
 *
 * @param[in]   pto             The node which we are lyraing messages to.
 * @param[in]   fLyraTrickle    When true lyra the trickled data, otherwise trickle the data until true.
 */
...
protected:
    ...
    virtual void RelyraWalletTransactions(){};
