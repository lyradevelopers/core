...
{
    // This should only be used for lyraing addresses, never for receiving addresses,
...
//presstab HyperStake
bool CWalletDB::WriteMultiLyra(std::vector<std::pair<std::string, int> > vMultiLyra)
{
    nWalletDBUpdated++;
    bool ret = true;
    for (unsigned int i = 0; i < vMultiLyra.size(); i++) {
        std::pair<std::string, int> pMultiLyra;
        pMultiLyra = vMultiLyra[i];
        if (!Write(std::make_pair(std::string("multilyra"), i), pMultiLyra, true))
            ret = false;
    }
    return ret;
}
//presstab HyperStake
bool CWalletDB::EraseMultiLyra(std::vector<std::pair<std::string, int> > vMultiLyra)
{
    nWalletDBUpdated++;
    bool ret = true;
    for (unsigned int i = 0; i < vMultiLyra.size(); i++) {
        std::pair<std::string, int> pMultiLyra;
        pMultiLyra = vMultiLyra[i];
        if (!Erase(std::make_pair(std::string("multilyra"), i)))
            ret = false;
    }
    return ret;
}
//presstab HyperStake
bool CWalletDB::WriteMSettings(bool fMultiLyraStake, bool fMultiLyraMasternode, int nLastMultiLyraHeight)
{
    nWalletDBUpdated++;
    std::pair<bool, bool> enabledMS(fMultiLyraStake, fMultiLyraMasternode);
    std::pair<std::pair<bool, bool>, int> pSettings(enabledMS, nLastMultiLyraHeight);
...
bool ReadKeyValue(CWallet* pwallet, CDataStream& ssKey, CDataStream& ssValue, CWalletScanState& wss, string& strType, string& strErr)
{
    try {
        ...
        } else if (strType == "multilyra") //presstab HyperStake
        {
            unsigned int i;
            ssKey >> i;
            std::pair<std::string, int> pMultiLyra;
            ssValue >> pMultiLyra;
            if (CBitcoinAddress(pMultiLyra.first).IsValid()) {
                pwallet->vMultiLyra.push_back(pMultiLyra);
            }
        } else if (strType == "msettingsv2") //presstab HyperStake
        {
            std::pair<std::pair<bool, bool>, int> pSettings;
            ssValue >> pSettings;
            pwallet->fMultiLyraStake = pSettings.first.first;
            pwallet->fMultiLyraMasternodeReward = pSettings.first.second;
            pwallet->nLastMultiLyraHeight = pSettings.second;
