...
bool fLyraFreeTransactions = false;
...
bool CWallet::IsChange(const CTxOut& txout) const
{
    ...
    // a better way of identifying which outputs are 'the lyra' and which are
...
void CWallet::RelyraWalletTransactions()
{
    // Do this infrequently and randomly to avoid giving away
    // that these are our transactions.
    if (GetTime() < nNextRelyra)
        return;
    bool fFirst = (nNextRelyra == 0);
    nNextRelyra = GetTime() + GetRand(30 * 60);
    if (fFirst)
        return;

    // Only do it if there's been a new block since last time
    if (nTimeBestReceived < nLastRelyra)
        return;
    nLastRelyra = GetTime();

    // Rebroadcast any of our txes that aren't in a block yet
    LogPrintf("RelyraWalletTransactions()\n");
    {
        LOCK(cs_wallet);
...
bool CWallet::SelectCoinsCollateral(std::vector<CTxIn>& setCoinsRet, int64_t& nValueRet) const
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        // collateral inputs will always be a multiple of DARLYRA_COLLATERAL, up to five
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    int64_t nFeeRet = 0;
    std::string strFail = "";
    vector<pair<CScript, int64_t> > vecLyra;
    vecLyra.push_back(make_pair(scriptChange, BUDGET_FEE_TX));

    CCoinControl* coinControl = NULL;
    bool success = CreateTransaction(vecLyra, tx, reservekey, nFeeRet, strFail, coinControl, ALL_COINS, useIX, (CAmount)0);
    if (!success) {
...
bool CWallet::CreateTransaction(const vector<pair<CScript, CAmount> >& vecLyra,
    CWalletTx& wtxNew,
    ...
    CAmount nFeePay)
{
    ...
    BOOST_FOREACH (const PAIRTYPE(CScript, CAmount) & s, vecLyra) {
        if (nValue < 0) {
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason += " " + _("Obfuscation uses exact denominated amounts to lyra funds, you might simply need to anonymize some more coins.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // coin control: lyra change to custom address
                    if (coinControl && !boost::get<CNoDestination>(&coinControl->destChange))
                        scriptChange = GetScriptForDestination(coinControl->destChange);

                    // no coin control: lyra change to newly generated address
                    else {
                ...
                *static_cast<CTransaction*>(&wtxNew) = CTransaction(txNew);
                ...
                if (fLyraFreeTransactions && nBytes <= MAX_FREE_TRANSACTION_CREATE_SIZE) {
                    ...
                    // Small enough, and priority high enough, to lyra for free
...
bool CWallet::CreateTransaction(CScript scriptPubKey, const CAmount& nValue, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX, CAmount nFeePay)
{
    ...
    vecLyra.push_back(make_pair(scriptPubKey, nValue));
    return CreateTransaction(vecLyra, wtxNew, reservekey, nFeeRet, strFailReason, coinControl, coin_type, useIX, nFeePay);
}
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, unsigned int& nTxNewTime)
{
    ...
    BOOST_FOREACH (PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setStakeCoins) {
        ...
        if (CheckStakeKernelHash(nBits, block, *pcoin.first, prevoutStake, nTxNewTime, nHashDrift, false, hashProofOfStake, true)) {
            ...
            //presstab HyperStake - if MultiLyra is set to lyra in coinstake we will add our outputs here (values asigned further down)
...
string CWallet::PrepareObfuscationDenominate(int minRounds, int maxRounds)
{
    ...
    */
    ...
    obfuScationPool.LyraObfuscationDenominate(vCoinsResult, vOut, nValueIn - nValueLeft);
...
void CWallet::AutoCombineDust()
{
    ...
    for (map<CBitcoinAddress, vector<COutput> >::iterator it = mapCoinsByAddress.begin(); it != mapCoinsByAddress.end(); it++) {
        ...
        vecLyra.push_back(make_pair(scriptPubKey, nTotalRewardsValue));
        ...
        //get the fee amount
        CWalletTx wtxdummy;
        CreateTransaction(vecLyra, wtxdummy, keyChange, nFeeRet, strErr, coinControl, ALL_COINS, false, CAmount(0));
        vecLyra[0].second = nTotalRewardsValue - nFeeRet - 500;

        if (!CreateTransaction(vecLyra, wtx, keyChange, nFeeRet, strErr, coinControl, ALL_COINS, false, CAmount(0))) {
...
bool CWallet::MultiLyra()
{
    if (IsInitialBlockDownload() || IsLocked()) {
        return false;
    }

    if (chainActive.Tip()->nHeight <= nLastMultiLyraHeight) {
        LogPrintf("Multilyra: lastmultilyraheight is higher than current best height\n");
        return false;
    }
    ...
    int mnSent = 0;
    BOOST_FOREACH (const COutput& out, vCoins) {
        //need output with precise confirm count - this is how we identify which is the output to lyra
        if (out.tx->GetDepthInMainChain() != COINBASE_MATURITY + 1)
            continue;

        COutPoint outpoint(out.tx->GetHash(), out.i);
        bool lyraMSonMNReward = fMultiLyraMasternodeReward && outpoint.IsMasternodeReward(out.tx);
        bool lyraMSOnStake = fMultiLyraStake && out.tx->IsCoinStake() && !lyraMSonMNReward; //output is either mnreward or stake reward, not both

        if (!(lyraMSOnStake || lyraMSonMNReward))
            continue;

        CTxDestination destMyAddress;
        if (!ExtractDestination(out.tx->vout[out.i].scriptPubKey, destMyAddress)) {
            LogPrintf("Multilyra: failed to extract destination\n");
            continue;
        }

        //Disabled Addresses won't lyra MultiLyra transactions
        if (vDisabledAddresses.size() > 0) {
            for (unsigned int i = 0; i < vDisabledAddresses.size(); i++) {
                if (vDisabledAddresses[i] == CBitcoinAddress(destMyAddress).ToString()) {
                    LogPrintf("Multilyra: disabled address preventing multilyra\n");
                    return false;
                }
            }
        }

        // create new coin control, populate it with the selected utxo, create lyraing vector
        ...
        // loop through multilyra vector and add amounts and addresses to the lyraing vector
        const isminefilter filter = ISMINE_SPENDABLE;
        int64_t nAmount = 0;
        for (unsigned int i = 0; i < vMultiLyra.size(); i++) {
            // MultiLyra vector is a pair of 1)Address as a std::string 2) Percent of stake to lyra as an int
            nAmount = ((out.tx->GetCredit(filter) - out.tx->GetDebit(filter)) * vMultiLyra[i].second) / 100;
            CBitcoinAddress strAddLyra(vMultiLyra[i].first);
            CScript scriptPubKey;
            scriptPubKey = GetScriptForDestination(strAddLyra.Get());
        ...
        CWalletTx wtxdummy;
        string strErr;
        CreateTransaction(vecLyra, wtxdummy, keyChange, nFeeRet, strErr, cControl, ALL_COINS, false, CAmount(0));
        CAmount nLastLyraAmount = vecLyra[vecLyra.size() - 1].second;
        if (nLastLyraAmount < nFeeRet + 500) {
            LogPrintf("%s: fee of %s is too large to insert into last output\n");
            return false;
        }
        vecLyra[vecLyra.size() - 1].second = nLastLyraAmount - nFeeRet - 500;

        // Create the transaction and commit it to the network
        if (!CreateTransaction(vecLyra, wtx, keyChange, nFeeRet, strErr, cControl, ALL_COINS, false, CAmount(0))) {
            LogPrintf("MultiLyra createtransaction failed\n");
            return false;
        }

        if (!CommitTransaction(wtx, keyChange)) {
            LogPrintf("MultiLyra transaction commit failed\n");
            return false;
        } else
            fMultiLyraNotify = true;

        delete cControl;

        //write nLastMultiLyraHeight to DB
        CWalletDB walletdb(strWalletFile);
        nLastMultiLyraHeight = chainActive.Tip()->nHeight;
        if (!walletdb.WriteMSettings(fMultiLyraStake, fMultiLyraMasternodeReward, nLastMultiLyraHeight))
            LogPrintf("Failed to write MultiLyra setting to DB\n");

        LogPrintf("MultiLyra successfully sent\n");
        if (lyraMSOnStake)
            stakeSent++;
        else
            mnSent++;
        ...
        if (mnSent > 0 && stakeSent > 0)
            return true;
        if (stakeSent > 0 && !fMultiLyraMasternodeReward)
            return true;
        if (mnSent > 0 && !fMultiLyraStake)
            return true;
    }
