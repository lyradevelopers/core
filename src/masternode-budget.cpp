...
void CBudgetManager::SubmitFinalBudget(std::vector<uint256> vBudgetHash)
{
    ...
    if (!mapCollateralTxids.count(tempBudget.GetHash())) {
        ...
        // Lyra the tx to the network. Do NOT use SwiftTx, locking might need too much time to propagate, especially for testnet
...
void CBudgetManager::Sync(CNode* pfrom, uint256 nProp, bool fPartial)
{
    ...
    /*
        ...
        budget object to see if they're OK. If all checks pass, we'll lyra it to the peer.
