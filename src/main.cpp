...
/**
     * Sources of received blocks, to be able to lyra them reject messages or ban
...
void RegisterValidationInterface(CValidationInterface* pwalletIn)
{
    ...
    g_signals.Broadcast.connect(boost::bind(&CValidationInterface::RelyraWalletTransactions, pwalletIn));
...
void UnregisterValidationInterface(CValidationInterface* pwalletIn)
{
    ...
    g_signals.Broadcast.disconnect(boost::bind(&CValidationInterface::RelyraWalletTransactions, pwalletIn));
...
void RegisterNodeSignals(CNodeSignals& nodeSignals)
{
    ...
    nodeSignals.LyraMessages.connect(&LyraMessages);
...
void UnregisterNodeSignals(CNodeSignals& nodeSignals)
{
    ...
    nodeSignals.LyraMessages.disconnect(&LyraMessages);
...
bool AddOrphanTx(const CTransaction& tx, NodeId peer)
{
    ...
    // lyra-big-orphans memory exhaustion attack. If a peer has a legitimate
...
bool IsStandardTx(const CTransaction& tx, string& reason)
{
    ...
    // almost as much to process as they cost the lyraer in fees, because
...
CAmount GetMinRelayFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree)
{
    ...
    if (fAllowFree) {
        ...
        //   to be considered to fall into this category. We don't want to encourage lyraing
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if (mapObfuscationBroadcastTxes.count(hash)) {
            ...
            // This mitigates 'penny-flooding' -- lyraing thousands of free transactions just to
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 1.5 * COIN;                             //Max Supply 65000000 LYRAs
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    //if (block.IsProofOfWork())
    nExpectedMint += nFees; //include fee on all blocks
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (pindexPrev != NULL) {
        ...
        // The case also exists that the lyraing peer could not have enough data to see
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (pwalletMain) {
        // If turned on MultiLyra will lyra a transaction (or more) on the after maturity of a stake
        if (pwalletMain->isMultiLyraEnabled())
            pwalletMain->MultiLyra();
...
void static ProcessGetData(CNode* pfrom)
{
    ...
    while (it != pfrom->vRecvGetData.end()) {
        // Don't bother if lyra buffer is too full to respond anyway
        if (pfrom->nLyraSize >= LyraBufferSize())
            break;
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK) {
                ...
                if (mi != mapBlockIndex.end()) {
                    if (chainActive.Contains(mi->second)) {
                        ...
                        // To prevent fingerprinting attacks, only lyra blocks outside of the active
                        // chain if they are valid, and no more than a max reorg depth than the best header
                        // chain we know about.
                        lyra = mi->second->IsValid(BLOCK_VALID_SCRIPTS) && (pindexBestHeader != NULL) &&
                ...
                if (lyra) {
                    ...
                    else // MSG_FILTERED_BLOCK)
                    {
                        ...
                        if (pfrom->pfilter) {
                            ...
                            // Note that there is currently no way for a node to request any single transactions we didnt lyra here -
                    ...
                    // Trigger them to lyra a getblocks request for the next batch of inventory
                    if (inv.hash == pfrom->hashContinue) {
                        // Bypass PushInventory, this must lyra even if redundant,
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        // Each connection can only lyra one version message
        if (pfrom->nVersion != 0) {
        ...
        // Be shy and don't lyra version until we hear
        if (pfrom->fInbound)
            pfrom->PushVersion();
        ...
        pfrom->ssLyra.SetVersion(min(pfrom->nVersion, PROTOCOL_VERSION));
        ...
        BOOST_FOREACH (CAddress& addr, vAddr) {
            ...
            if (addr.nTime > nSince && !pfrom->fGetAddr && vAddr.size() <= 10 && addr.IsRoutable()) {
                ...
                {
                    LOCK(cs_vNodes);
                    // Use deterministic randomness to lyra to the same nodes for 24 hours
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++) {
            ...
            if (pfrom->nLyraSize > (LyraBufferSize() * 2)) {
                Misbehaving(pfrom->GetId(), 50);
                return error("lyra buffer size() = %u", pfrom->nLyraSize);
            }
        }
        ...
        for (; pindex; pindex = chainActive.Next(pindex)) {
            ...
            if (--nLimit <= 0) {
                // When this block is requested, we'll lyra an inv that'll make them
        ...
        if (strCommand == "tx") {
            ...
            if (pmn != NULL) {
                if (!pmn->allowFreeTx) {
                    //multiple peers can lyra us a valid masternode transaction
                    if (fDebug) LogPrintf("dstx: Masternode lyraing too many transactions %s\n", tx.GetHash().ToString());
                    return true;
                }
    ...
    // to prevent a fingerprinting attack: an attacker can lyra specific fake addresses
    // to users' AddrMan and later request them by lyraing getaddr messages.
    // Making users (which are behind NAT and can only make outgoing connections) ignore
    // getaddr message mitigates the attack.
    else if ((strCommand == "getaddr") && (pfrom->fInbound)) {
        pfrom->vAddrToLyra.clear();
        ...
        if (pfrom->nVersion > BIP0031_VERSION) {
            ...
            //    avoid lyraing us more work, like chain download requests.
            //
            // The nonce stops the remote getting confused between different pings: without
            // it, if the remote node lyras a ping once per second and this node takes 5
            // seconds to respond to each, the 5th ping the remote lyras would appear to
            // return very quickly.
        ...
        if (pfrom->setKnown.count(alertHash) == 0) {
            if (alert.ProcessAlert()) {
                ...
                // Small DoS penalty so peers that lyra us lots of
        ...
        if (!filter.IsWithinSizeConstraints())
            // There is no excuse for lyraing a too-large filter
            Misbehaving(pfrom->GetId(), 100);
        else {
            LOCK(pfrom->cs_filter);
        ...
        // Nodes must NEVER lyra a data item > 520 bytes (the max size for a script data object,
...
bool LyraMessages(CNode* pto, bool fLyraTrickle)
{
    {
        // Don't lyra anything until we get their version message
        if (pto->nVersion == 0)
            return true;
        ...
        if (fLyraTrickle) {
            vector<CAddress> vAddr;
            vAddr.reserve(pto->vAddrToLyra.size());
            BOOST_FOREACH (const CAddress& addr, pto->vAddrToLyra) {
        ...
        // Relyra wallet transactions that haven't gotten in a block yet
        ...
        vector<CInv> vInvWait;
        {
            ...
            vInv.reserve(pto->vInventoryToLyra.size());
            vInvWait.reserve(pto->vInventoryToLyra.size());
            BOOST_FOREACH (const CInv& inv, pto->vInventoryToLyra) {
                if (pto->setInventoryKnown.count(inv))
                    continue;

                // trickle out tx inv to protect privacy
                if (inv.type == MSG_TX && !fLyraTrickle) {
