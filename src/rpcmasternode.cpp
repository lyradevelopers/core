...
void LyraMoney(const CTxDestination& address, CAmount nValue, CWalletTx& wtxNew, AvailableCoinsType coin_type = ALL_COINS)
{
    // Check amount
    if (nValue <= 0)
...
Value obfuscation(const Array& params, bool fHelp)
{
    ...
    // Wallet comments
    CWalletTx wtx;
    //    string strError = pwalletMain->LyraMoneyToDestination(address.Get(), nAmount, wtx, ONLY_DENOMINATED);
    LyraMoney(address.Get(), nAmount, wtx, ONLY_DENOMINATED);
    //    if (strError != "")
    //        throw JSONRPCError(RPC_WALLET_ERROR, strError);
